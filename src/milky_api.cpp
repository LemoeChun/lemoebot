...
cpr::Response SendMsg(json &msg){
    ...
    if (msg["message"][0]["type"] == "text" 
            && msg["message"][0]["data"]["text"].get<std::string>().length() > 500 ) {
        json forward_msg = msg;
        forward_msg["message"][0]["type"] = "forward";
        forward_msg["message"][0]["data"]["messages"]["segments"] = msg["message"];
        forward_msg["message"][0]["user_id"] = config["Bot"]["QQ_ID"].value_or<uint>(1);
        forward_msg["message"][0]["sender_name"] = config["Bot"]["QQ_Name"].value_or<std::string>("喵呜");
    }
    HX::log::hxLog.info("将向 ",api_url, " 发送：\n",msg.dump(4));
    return cpr::Post(cpr::Url{api_url},json_header,cpr::Body{msg.dump()});
}
